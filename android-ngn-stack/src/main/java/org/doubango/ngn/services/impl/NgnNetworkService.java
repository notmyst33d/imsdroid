...
public class NgnNetworkService  extends NgnBaseService implements INgnNetworkService {
	...
	private boolean requestCellularRouteToHostApi8(String host, boolean IPv6)
	{
		...
		int resultInt; // Will be set in try-catch

		// Activate the mobile connection if not already and do nothing if already done
		try {
			java.lang.reflect.Method method = connectivityManager.getClass().getMethod("startUsingNetworkFeature", int.class, String.class);
			resultInt = (int) method.invoke(connectivityManager, ConnectivityManager.TYPE_MOBILE, "enableHIPRI");
		} catch(Exception e) {
			Log.d(TAG, "startUsingNetworkFeature(TYPE_MOBILE, enableHIPRI) reflection failed");
			return false;
		}
	...
	private boolean bindProcessToConnectionApi8(final NgnNetworkConnection connection)
	{
		...
		try {
			java.lang.reflect.Method method = connectivityManager.getClass().getMethod("requestRouteToHost", int.class, String.class);
			return (boolean) method.invoke(connectivityManager, ConnectivityManager.TYPE_MOBILE_HIPRI, "enableHIPRI");
		} catch(Exception e) {
			Log.d(TAG, "startUsingNetworkFeature(TYPE_MOBILE, enableHIPRI) reflection failed");
			return false;
		}
